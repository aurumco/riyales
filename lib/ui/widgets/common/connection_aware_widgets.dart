...
// --- NetworkAwareWidget ---
class NetworkAwareWidget extends StatefulWidget { // Changed to StatefulWidget
  ...
  @override
  State<NetworkAwareWidget> createState() => _NetworkAwareWidgetState(); // Changed
}

class _NetworkAwareWidgetState extends State<NetworkAwareWidget> { // Changed from ConsumerState
  ...
  void initState() {
    ...
    _currentStatus = connectionService.isOnline ? ConnectionStatus.connected : ConnectionStatus.internetDown;
    ...
    if (widget.checkOnInit) {
       WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) { // Ensure it's still mounted
    ...
    _subscription = connectionService.statusStream.listen((status) {
      if (_currentStatus != status) {
        ...
        if (mounted && context.mounted) { // Ensure context is valid
  ...
  Future<void> _checkConnectionOnInitWithProvider() async {
    ...
    final appConfig = context.read<AppConfig>(); // Using Provider to read AppConfig
    if (appConfig.remoteConfigUrl.isNotEmpty) { // Check if it's not the default empty config
      await connectionService.checkConnection(appConfig.apiEndpoints.currencyUrl);
    } else {
