...
import '../widgets/common/title_with_language_transition.dart';
...
class HomeScreenState extends State<HomeScreen> // Changed from ConsumerState
  ...
  void _setupScrollListener(int tabIndex) {
    if (_tabScrollListeners.containsKey(tabIndex)) {
      ...
      if (controller != null && controller.hasClients) { // Ensure controller has clients
  ...
  void _setupTabs() {
    ...
    _mainTabViews.addAll([
      ...
      StockPage(key: stockTabKey, showSearchBar: _showSearchBar, isSearchActive: _isSearchActive),
    ]);

    int previousIndex = _tabController.index; // Save before disposing
    if (_tabController.hasListeners) { // Check if _tabController was already initialized
        previousIndex = _tabController.index;
        _tabController.removeListener(_handleTabSelection); // Remove old listener
    }
    ...
    _tabController = TabController(
        ...
        vsync: this,
        initialIndex: previousIndex != null && previousIndex < _mainTabs.length ? previousIndex : 0,
    );

    if (!_tabListenerAdded) { // This logic might need re-evaluation
  ...
  void _handleTabSelection() {
      if (!_tabController.indexIsChanging && mounted) { // check mounted
          _initializeTab(_tabController.index);
      }
  }
  ...
  void didChangeDependencies() {
    ...
    // Initialize TabController here as it depends on TickerProviderStateMixin
    // and potentially Localizations if tab labels are dynamic.
    if (!_tabController.hasListeners) { // First time or after dispose
        _tabController = TabController(length: 4, vsync: this); // Default length, will be updated
    }
  ...
  void dispose() {
    ...
    _tabScrollControllers.values.where((c) => c != null).forEach((c) => c!.dispose()); // Dispose scroll controllers
  ...
  Widget build(BuildContext context) {
    ...
    if (appConfig.appName == "Riyales Default Fallback") { // Check if it's the default fallback from FutureProvider
        return const Scaffold(body: Center(child: Text("App configuration is using fallback.")));
    }
    ...
    final isDarkMode = Theme.of(context).brightness == Brightness.dark; // This is fine
    ...
    final segmentActiveBackground = isDarkMode ? tealGreen.withAlpha((255 * 0.15).round()) : Theme.of(context).colorScheme.secondaryContainer.withAlpha(128); // Adjusted opacity
    final segmentActiveTextColor = isDarkMode ? tealGreen.withAlpha((255 * 0.9).round()) : Theme.of(context).colorScheme.onSecondaryContainer; // Adjusted opacity
    ...
    Widget mainScaffold = Scaffold(
      appBar: AppBar(
        title: TitleWithLanguageTransition(
            ...
            isRTL: localeNotifier.locale.languageCode == 'fa', // Using localeNotifier
        ),
        actions: [
          AnimatedAlign(
    ...
    Widget mainScaffold = Scaffold(
      appBar: AppBar(
        ...
        actions: [
          AnimatedAlign(
            alignment: Localizations.localeOf(context).languageCode == 'fa' ? Alignment.centerLeft : Alignment.center,
            duration: const Duration(milliseconds: 300),
            curve: Curves.easeInOutQuart,
            child: IconButton(
              icon: AnimatedSwitcher(
                ...
                child: Icon(
                  _isSearchActive ? CupertinoIcons.clear : CupertinoIcons.search,
                  key: ValueKey<bool>(_isSearchActive),
                  color: _showSearchBar ? (isDarkMode ? Colors.grey[400] : Colors.grey[600]) : (isDarkMode ? Colors.white : Colors.black),
                  size: 28,
                ),
              ),
              onPressed: () {
                ...
                final currentTabIndex = _tabController.index;
                _tabScrollControllers[currentTabIndex] ??= _findScrollController(currentTabIndex);
          ...
          AnimatedAlign(
            ...
            child: Padding(
              ...
              child: GestureDetector(
                ...
                child: Container(
                  width: 40, height: 40, decoration: const BoxDecoration(shape: BoxShape.circle),
                  child: Icon(CupertinoIcons.person_crop_circle, size: 28, color: Theme.of(context).colorScheme.onSurface),
                ),
              ),
            ),
          ),
          ...
          child: Padding(
            ...
            child: LayoutBuilder(
              builder: (context, constraints) {
                ...
                final tabRadius = themeConfig.cardBorderRadius * 0.7;
                return Row(
                  mainAxisSize: isMobile ? MainAxisSize.max : MainAxisSize.min,
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: List.generate(4, (index) {
                    final isSelected = _tabController.index == index;
                    final label = [l10n.tabCurrency, l10n.tabGold, l10n.tabCrypto, l10n.tabStock][index];
                    void onTabTap() {
                      if (_tabController.index == index) {
                        final controller = _tabScrollControllers[index] ??= _findScrollController(index);
                        if (controller != null && controller.hasClients) {
                          controller.jumpTo(controller.offset);
                      ...
                      child: Padding(
                        ...
                        child: Center(
                          child: Builder(builder: (context) {
                            Widget textWidget = Text(label, style: isSelected ? selectedTextStyle : unselectedTextStyle, textAlign: TextAlign.center);
                            Widget fittedText = FittedBox(fit: BoxFit.scaleDown, child: textWidget);
                            if (isSelected && !isDarkMode) fittedText = Transform.translate(offset: const Offset(0, 1), child: fittedText);
                            return fittedText;
                          }),
                        ),
                      ),
                        ...
                        showCupertinoModalPopup(
                          ...
                          builder: (_) => CupertinoTheme(
                            ...
                            child: CupertinoActionSheet(
                              title: Text(isFa ? 'مرتب‌سازی' : 'Sort By', style: TextStyle(fontFamily: isFa ? 'Vazirmatn' : 'SF-Pro', fontSize: 17, fontWeight: FontWeight.w600, color: isDarkMode ? Colors.white : Colors.black)),
                              ...
                              cancelButton: CupertinoActionSheetAction(
                                ...
                                child: Text(isFa ? 'انصراف' : 'Cancel', style: TextStyle(fontFamily: isFa ? 'Vazirmatn' : 'SF-Pro', fontSize: 17, fontWeight: FontWeight.w600, color: tealGreen)),
                              ),
                            ),
                          ),
                        );
                    ...
                    return isMobile ? Expanded(child: wrapped) : Padding(padding: EdgeInsets.symmetric(horizontal: horizontalMargin), child: wrapped);
                  }),
                );
              },
            ),
      ...
      body: Column(
        children: [
          if (_tabController.index != 3) // Search bar not shown on stock page (it has its own)
            AnimatedContainer(
              duration: _showSearchBar ? const Duration(milliseconds: 400) : const Duration(milliseconds: 300),
              curve: Curves.easeInOutQuart,
              height: _showSearchBar ? 48.0 : 0.0,
              margin: _showSearchBar ? const EdgeInsets.only(top: 10.0, bottom: 4.0) : EdgeInsets.zero,
              padding: const EdgeInsets.symmetric(horizontal: 12),
              color: Theme.of(context).scaffoldBackgroundColor,
              child: AnimatedOpacity(
                opacity: _showSearchBar ? 1.0 : 0.0,
                duration: _showSearchBar ? const Duration(milliseconds: 300) : const Duration(milliseconds: 200),
                child: _isSearchActive ? Builder(builder: (context) {
                  final searchQueryNotifier = context.watch<SearchQueryNotifier>(); // Using Provider
                  final searchText = searchQueryNotifier.query;
                  final isRTL = Localizations.localeOf(context).languageCode == 'fa' || containsPersian(searchText);
                  final textColor = isDarkMode ? Colors.grey[300] : Colors.grey[700];
                  final placeholderColor = isDarkMode ? Colors.grey[600] : Colors.grey[500];
                  final iconColor = isDarkMode ? Colors.grey[400] : Colors.grey[600];
                  final fontFamily = isRTL ? 'Vazirmatn' : 'SF-Pro';
                  return CupertinoTextField(
                    controller: TextEditingController(text: searchText)..selection = TextSelection.fromPosition(TextPosition(offset: searchText.length)),
                    onChanged: (v) => context.read<SearchQueryNotifier>().query = v, // Using Provider
                    placeholder: l10n.searchPlaceholder,
                    placeholderStyle: TextStyle(color: placeholderColor, fontFamily: fontFamily),
                    prefix: Padding(padding: const EdgeInsetsDirectional.only(start: 18), child: Icon(CupertinoIcons.search, size: 20, color: iconColor)),
                    suffix: searchText.isNotEmpty ? CupertinoButton(
                      padding: const EdgeInsetsDirectional.only(end: 18), minSize: 30,
                    ...
                    padding: EdgeInsetsDirectional.only(start: 9, end: searchText.isNotEmpty ? 28 : 12, top: 8, bottom: 8),
                    style: TextStyle(color: textColor, fontFamily: fontFamily),
                    cursorColor: isDarkMode ? Colors.grey[400] : Colors.grey[700],
                    decoration: BoxDecoration(color: isDarkMode ? const Color(0xFF2C2C2E) : const Color(0xFFE2E2E6), borderRadius: BorderRadius.circular(10)),
                  );
                }) : const SizedBox(),
              ),
            ),
          Expanded(
            child: AnimatedBuilder( // Using AnimatedBuilder to react to TabController changes
              ...
              builder: (context, child) {
                ...
                return AnimatedSwitcher(
                  ...
                  transitionBuilder: (Widget c, Animation<double> a) => FadeTransition(opacity: a, child: c),
                  child: Container(
    ...
    try {
      switch (tabIndex) {
        ...
        case 3:
          final stockState = stockTabKey.currentState;
          if (stockState != null) {
            final activeStockTabIndex = stockState.stockTabController.index; // Getter needed
            // Ensure stockScrollControllers is accessible or a method to get it
            return stockState.stockScrollControllers[activeStockTabIndex]; // Getter needed
          }
          return null;
        default: return null;
      }
