...
class CryptoDataNotifier extends ChangeNotifier {
  ...
  Future<void> fetchInitialData({bool isRefresh = false, bool isLoadMore = false, List<String>? favoriteIds}) async {
    ...
    if (isLoadMore) {
      ...
      if (isSpecialFetch || _isLoadingMore || (_fullDataList.isNotEmpty && cryptoAssets.length >= _fullDataList.length)) {
        return;
      }
      _isLoadingMore = true;
    ...
    try {
      ...
      if (!isOnline) {
        error = "Offline";
        if (!isRefresh && !isSpecialFetch && !hasDataBeenFetchedOnce) { // Check !hasDataBeenFetchedOnce for initial offline
        ...
        if (isSpecialFetch) {
          ...
          final List<String> priorityList = appConfig.priorityCrypto; // Use pre-loaded list
          ...
          if (priorityList.isNotEmpty) {
            for (final name in priorityList) {
              ...
              final assetsToPrio = fetchedAssets.where((a) => // Renamed to avoid conflict
                  a.name.toLowerCase() == lowerName &&
                  !usedAssetIds.contains(a.id));
              for (final assetToPrio in assetsToPrio) { // Renamed to avoid conflict
          ...
          cryptoAssets = _fullDataList.take(appConfig.initialItemsToLoad).toList();
        }
