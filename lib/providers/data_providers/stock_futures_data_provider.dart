...
class StockFuturesDataNotifier extends ChangeNotifier {
  ...
  StockFuturesDataNotifier({required this.apiService, required this.appConfig, required this.connectionService}) {
    ...
    try {
      ...
      try {
        ...
        if (priorityResponse is Map<String, dynamic>) {
          priorityList = List<String>.from(priorityResponse['stock_futures'] as List<dynamic>? ?? []);
        }
      } catch (_) {
