...
class StockTseIfbDataNotifier extends ChangeNotifier {
  ...
  StockTseIfbDataNotifier({required this.apiService, required this.appConfig, required this.connectionService}) {
  ...
  Future<void> fetchInitialData({bool isRefresh = false, bool isLoadMore = false}) async {
    ...
    isLoading = true;
    if (isRefresh || isSpecialFetch) { // isSpecialFetch will always be false here
      error = null;
    }
    notifyListeners();
      ...
      if (!isOnline) {
        ...
        try {
          ...
          if (priorityResponse is Map<String, dynamic>) {
            priorityList = List<String>.from(priorityResponse['stock_tse_ifb_symbols'] as List<dynamic>? ?? []);
          }
        } catch (_) {
