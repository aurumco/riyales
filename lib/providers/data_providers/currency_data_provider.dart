...
class CurrencyDataNotifier extends ChangeNotifier {
  ...
  Future<void> fetchInitialData({bool isRefresh = false, bool isLoadMore = false}) async {
    ...
    if (isLoadMore) {
      if (_isLoadingMore || (_fullDataList.isNotEmpty && currencyAssets.length >= _fullDataList.length)) {
        return;
      }
      _isLoadingMore = true;
    ...
    if (!isRefresh && !isSpecialFetch && hasDataBeenFetchedOnce) {
      if (isLoading) { // If called while already loading (e.g. from constructor then immediately from UI)
    ...
    try {
      ...
      if (!isOnline) {
        ...
        final List<String> priorityList = appConfig.priorityCurrency; // Use pre-loaded list
        ...
        final List<CurrencyAsset> priorityAssetsList = []; // Renamed to avoid conflict
        ...
        if (priorityList.isNotEmpty) {
          ...
          for (final assetInFetched in fetchedAssets) { // Renamed asset to assetInFetched
        ...
        currencyAssets = _fullDataList.take(appConfig.initialItemsToLoad).toList();
        error = null;
      }

      if (error == null && !isSpecialFetch) { // Only update if fetch was successful and not a special fetch
