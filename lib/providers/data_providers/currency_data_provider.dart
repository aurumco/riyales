...
class CurrencyDataNotifier extends ChangeNotifier {
  ...
  CurrencyDataNotifier({required this.apiService, required this.appConfig, required this.connectionService}) {
  ...
  Future<void> fetchInitialData({bool isRefresh = false}) async {
    ...
    final bool isOnline = await connectionService.checkConnection(appConfig.apiEndpoints.currencyUrl);
    if (!isOnline) {
    ...
    try {
      final dynamic responseData = await apiService.fetchData(appConfig.apiEndpoints.currencyUrl);
      ...
      try {
        ...
        if (priorityResponse is Map<String, dynamic>) {
          priorityList = List<String>.from(priorityResponse['currency'] as List<dynamic>? ?? []);
        }
      } catch (_) {
      ...
      if (priorityList.isNotEmpty) {
        ...
        for (final asset in fetchedAssets) {
          if (!priorityAssets.any((pa) => pa.id == asset.id)) { // Check by ID for uniqueness
      ...
      isLoading = false;
      notifyListeners();
      if (!isRefresh) { // Only start auto-refresh on initial load or if it was stopped
        _startAutoRefresh();
      }
    }
  }
