    ...
    MultiProvider(
      providers: [
        ...
        ChangeNotifierProvider<LocaleNotifier>(
          create: (_) => LocaleNotifier(const Locale('en')), // Default, prefs will override
        ),
        ...
        ProxyProvider<AppConfig, ConnectionService>(
          update: (context, appConfig, previousConnectionService) {
            ...
            final cs = ConnectionService(); // It's a singleton, so this gets the instance
            if (appConfig.appName != "Riyales Default Fallback" && appConfig.remoteConfigUrl.isNotEmpty) {
        ...
        ProxyProvider2<Dio, AppConfig, ApiService>(
          update: (context, dio, appConfig, previous) => ApiService(dio, appConfig.apiEndpoints),
        ),

        ChangeNotifierProxyProvider<AppConfig, CardCornerSettingsNotifier>(
          create: (context) => CardCornerSettingsNotifier(Provider.of<AppConfig>(context, listen: false)),
          update: (context, appConfig, previousNotifier) => CardCornerSettingsNotifier(appConfig),
        ),

        ChangeNotifierProxyProvider3<AppConfig, ApiService, ConnectionService, CurrencyDataNotifier>(
          create: (context) => CurrencyDataNotifier(
            appConfig: Provider.of<AppConfig>(context, listen: false),
            apiService: Provider.of<ApiService>(context, listen: false),
            connectionService: Provider.of<ConnectionService>(context, listen: false),
          ),
          update: (context, appConfig, apiService, connService, previous) =>
              CurrencyDataNotifier(appConfig: appConfig, apiService: apiService, connectionService: connService),
        ),
        ChangeNotifierProxyProvider3<AppConfig, ApiService, ConnectionService, GoldDataNotifier>(
          ...
          update: (context, appConfig, apiService, connService, previous) =>
              GoldDataNotifier(appConfig: appConfig, apiService: apiService, connectionService: connService),
        ),
        ChangeNotifierProxyProvider3<AppConfig, ApiService, ConnectionService, CryptoDataNotifier>(
          ...
          update: (context, appConfig, apiService, connService, previous) =>
              CryptoDataNotifier(appConfig: appConfig, apiService: apiService, connectionService: connService),
        ),
        ChangeNotifierProxyProvider3<AppConfig, ApiService, ConnectionService, StockTseIfbDataNotifier>(
          ...
          update: (context, appConfig, apiService, connService, previous) =>
              StockTseIfbDataNotifier(appConfig: appConfig, apiService: apiService, connectionService: connService),
        ),
        ChangeNotifierProxyProvider3<AppConfig, ApiService, ConnectionService, StockDebtSecuritiesDataNotifier>(
          ...
          update: (context, appConfig, apiService, connService, previous) =>
              StockDebtSecuritiesDataNotifier(appConfig: appConfig, apiService: apiService, connectionService: connService),
        ),
        ChangeNotifierProxyProvider3<AppConfig, ApiService, ConnectionService, StockFuturesDataNotifier>(
          ...
          update: (context, appConfig, apiService, connService, previous) =>
              StockFuturesDataNotifier(appConfig: appConfig, apiService: apiService, connectionService: connService),
        ),
        ChangeNotifierProxyProvider3<AppConfig, ApiService, ConnectionService, StockHousingFacilitiesDataNotifier>(
          ...
          update: (context, appConfig, apiService, connService, previous) =>
              StockHousingFacilitiesDataNotifier(appConfig: appConfig, apiService: apiService, connectionService: connService),
        ),

        ChangeNotifierProxyProvider3<AppConfig, ApiService, LocaleNotifier, TermsNotifier>(
            create: (context) => TermsNotifier(
                appConfig: Provider.of<AppConfig>(context, listen: false),
                apiService: Provider.of<ApiService>(context, listen: false),
                languageCode: Provider.of<LocaleNotifier>(context, listen: false).locale.languageCode,
            ),
            update: (context, appConfig, apiService, localeNotifier, previous) => TermsNotifier(
                appConfig: appConfig,
...
class RiyalesApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (appConfig.appName == AppConfig.defaultConfig().appName && appConfig.remoteConfigUrl == AppConfig.defaultConfig().remoteConfigUrl) {
    ...
    final themeData = materialTheme.copyWith( // Global overrides
    ...
    return Theme(
      ...
      child: MaterialApp(
            ...
            theme: ui_theme_pkg.AppTheme.getThemeData(
              ...
              localeNotifier.locale.languageCode == 'fa' ? appConfig.fonts.persianFontFamily : appConfig.fonts.englishFontFamily,
              localeNotifier.locale.languageCode == 'fa' ? 'Vazirmatn' : 'Onest', // Title font
              false, // isDarkMode
            ),
            ...
            supportedLocales: appConfig.supportedLocales.map((loc) => Locale(loc)),
            ...
            builder: (context, child) { // Optional: For global AnimatedTheme or other wrappers
